...
def home():
    return render_template('painel/home.html', usuario=current_user)
...
def home_solicitante():
    return render_template('painel/home_solicitante.html', usuario=current_user)
...
def home_consultor():
    return render_template('painel/home_consultor.html', usuario=current_user)
...
@login_required
def almoxarifado():
    return render_template('painel/almoxarifado.html', usuario=current_user)

# --- Página ND / Grupos / UL ---
@main.route('/nd_grupos_ul')
@login_required
def nd_grupos_ul():
    return redirect(url_for('main.home'))  # Redirecionando pois o template foi removido

# --- Página de instrução de troca de senha ---
@main.route('/esqueci_senha')
def esqueci_senha():
    return render_template('usuarios/esqueci_senha.html')

# --- Página de Relatórios (botões desativados por padrão) ---
@main.route('/relatorios')
@login_required
def relatorios():
    return render_template('relatorios/index.html', usuario=current_user)
